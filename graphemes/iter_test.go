...
func TestIterMatchesSegmenter(t *testing.T) {
	...
	seg1 := graphemes.NewSegmenter(file)
	var expected [][]byte
	for seg1.Next() {
		expected = append(expected, seg1.Bytes())
	}

	iter := graphemes.Split(file)
	var got [][]byte
	for word := range iter {
		got = append(got, word)
	}
...
func TestIterMatchesScanner(t *testing.T) {
	...
	sc1 := graphemes.NewScanner(file1)
	var expected [][]byte
	for sc1.Scan() {
	...
	if len(got) == 0 || len(expected) != len(got) {
		t.Fatal("graphemes iter and scanner return different results")
	}
