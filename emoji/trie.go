...
var (
	bEmoji                 uint32 = 1
	bEmoji_Component       uint32 = 2
	bEmoji_Modifier        uint32 = 4
	bEmoji_Modifier_Base   uint32 = 8
	bEmoji_Presentation    uint32 = 16
	bExtended_Pictographic uint32 = 32
)

// lookup returns the trie value for the first UTF-8 encoding in s and
// the width in bytes of this encoding. The size will be 0 if s does not
// hold enough bytes to complete the encoding. len(s) must be greater than 0.
func (t *emojiTrie) lookup(s []byte) (v uint32, sz int) {
	c0 := s[0]
	switch {
...
func (t *emojiTrie) lookupUnsafe(s []byte) uint32 {
	c0 := s[0]
	if c0 < 0x80 { // is ASCII
...
// the width in bytes of this encoding. The size will be 0 if s does not
// hold enough bytes to complete the encoding. len(s) must be greater than 0.
func (t *emojiTrie) lookupString(s string) (v uint32, sz int) {
	c0 := s[0]
	switch {
	case c0 < 0x80: // is ASCII
		return emojiValues[c0], 1
...
// s must start with a full and valid UTF-8 encoded rune.
func (t *emojiTrie) lookupStringUnsafe(s string) uint32 {
	c0 := s[0]
	if c0 < 0x80 { // is ASCII
		return emojiValues[c0]
	}
...
// emojiTrie. Total size: 19968 bytes (19.50 KiB). Checksum: 47d09e2e81a66e7f.
...
func (t *emojiTrie) lookupValue(n uint32, b byte) uint32 {
	switch {
	default:
		return uint32(emojiValues[n<<6+uint32(b)])
	}
}

// emojiValues: 67 blocks, 4288 entries, 17152 bytes
// The third block is the zero block.
var emojiValues = [4288]uint32{
...
// emojiIndex: 11 blocks, 704 entries, 2816 bytes
// Block 0 is the zero block.
var emojiIndex = [704]uint32{
	// Block 0x0, offset 0x0
	// Block 0x1, offset 0x40
	// Block 0x2, offset 0x80
	// Block 0x3, offset 0xc0
