...
func (p prop) generateTrie() error {
	...
	const extendedPict = "Extended_Pictographic"
	if p.name == "Emoji" {
		extendedPictographic = runesByProperty[extendedPict]
	...
	if p.name == "Word" || p.name == "Grapheme" {
		...
		runesByProperty[extendedPict] = extendedPictographic
	}

	if p.name == "Word" {
		// defining word-like as containing one of the following
		wordlike := []*unicode.RangeTable{
			unicode.Letter, unicode.Number, unicode.Symbol,
		}
...
func getRunesFromRangeTable(rts ...*unicode.RangeTable) []rune {
	var result []rune

	for _, rt := range rts {
		// https://stackoverflow.com/questions/20176024/how-does-unicode-rangetable-work
...
func writeTrie(prop prop, runesByProperty map[string][]rune) error {
