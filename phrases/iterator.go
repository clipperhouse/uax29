...
// Iterate while Next() is true, and access the phrase via Value().
...
// Iterator is a generic iterator for phrases in strings or byte slices.
...
// Next advances the iterator to the next phrase.
// Returns false when there are no more phrases.
func (iter *Iterator[T]) Next() bool {
	...
	// These characters are guaranteed to stay together in a phrase (per WB8-10)
...
// Value returns the current phrase.
func (iter *Iterator[T]) Value() T {
	return iter.data[iter.start:iter.pos]
}

// Start returns the byte position of the current phrase in the original data.
func (iter *Iterator[T]) Start() int {
	return iter.start
}

// End returns the byte position after the current phrase in the original data.
...
// First returns the first phrase without advancing the iterator.
