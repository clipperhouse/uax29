...
// From https://github.com/blevesearch/segment/blob/master/segment_phrases_test.go
...
var letter filter.Func = func(token []byte) bool {
	return filter.AlphaNumeric(token) && !phrases.BleveNumeric(token)
}

var number filter.Func = phrases.BleveNumeric
var ideo filter.Func = phrases.BleveIdeographic

var none filter.Func = func(token []byte) bool {
	return !filter.AlphaNumeric(token) && !phrases.BleveNumeric(token) && !phrases.BleveIdeographic(token)
}
